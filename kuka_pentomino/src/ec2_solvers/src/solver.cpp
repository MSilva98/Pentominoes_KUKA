...
namespace ec2
{
    ...
    void Solver::getBasePosFromPixel(Point2d pixel, Eigen::Vector3d &pos, image_geometry::PinholeCameraModel model){
        double Z = -0.082;
        // Rectify Point
        pixel = model.rectifyPoint(pixel);
        cout << pixel << endl;
        // Convert 2d point to 3d ray passing in point P
        Point3d v;
        v = model.projectPixelTo3dRay(pixel);   // The pixel corresponds to a ray that starts at your camera position and passes through the point P (in your camera frame)
        cout << "3d Ray: " << v << endl;
    ...
    void Solver::solve()
    {
        ...
        Eigen::Vector3d posP5;
        
        getBasePosFromPixel(center, posP5, modelPT);
        posP5.z() = 0.5;       // Define z higher to see the whole pieces
        cout << posP5.transpose() << endl;
        lookAt(posP5, 0, 0.2, true);
        ...
        // From HERE DETECT EACH PIECE POSITION AND SEND ARM
    }
